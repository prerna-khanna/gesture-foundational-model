...
class SemanticEmbedder:
    ...
    def visualize_embeddings_with_pca(self, descriptions, labels=None):
        # Get embeddings
        ...
        if labels is None:
            labels = [f"Item {i+1}" for i in range(len(descriptions))]
        
        # Create color map based on categorical types from descriptions
        colors = []
        for desc in descriptions:
            if "directional" in desc:
                colors.append('blue')
            elif "rotational" in desc:
                colors.append('red')
            elif "complex" in desc.split(',')[0]:  # Check if primary type is complex
                colors.append('green')
            elif "shape" in desc:
        ...
        for i, (x, y) in enumerate(embeddings_2d):
            plt.scatter(x, y, color=colors[i], s=70)
        ...
        #plt.title(f"2D PCA of BERT Embeddings ({self.pooling_strategy.upper()} Pooling)\n"
        #         f"Explained Variance: {explained_variance[0]:.2%}, {explained_variance[1]:.2%}")
        plt.xlabel(f"PC1")# ({explained_variance[0]:.2%})")
        plt.ylabel(f"PC2 ")#({explained_variance[1]:.2%})")
        
        # Add annotations for clusters
        #self.annotate_clusters(embeddings_2d, descriptions)
        
        plt.tight_layout()
        plt.savefig(f"bert_embeddings_pca_{self.pooling_strategy}_no_annotate.pdf", dpi=300)
        plt.close()
        
        return embeddings_2d, pca
    
    def annotate_clusters(self, embeddings_2d, descriptions):
        # Try to identify and annotate clusters based on gesture types
        gesture_types = {
            "directional": [],
            "rotational": [],
            "shape": [],
        ...
        for i, desc in enumerate(descriptions):
            for gesture_type in gesture_types.keys():
                if gesture_type in desc:
                    gesture_types[gesture_type].append(i)
                    break
        
        # Calculate centroids for each type if there are points of that type
        for gesture_type, indices in gesture_types.items():
            if indices:
                points = embeddings_2d[indices]
                centroid = np.mean(points, axis=0)
                
                # Add a light background for the cluster label
                plt.text(centroid[0], centroid[1], f"{gesture_type.upper()} GESTURES",
...
if __name__ == "__main__":
    ...
    # Create labels for better readability in the visualization
    short_labels = [
        "Up", "Down", "Left", "Right",
        "CW Rotation", "CCW Rotation",
        "Up Jerk", "Down Jerk", "Left Jerk", "Right Jerk",
        "Square", "Circle", "Triangle", "Question Mark", "Figure 8"
    ]
    ...
    for strategy in strategies:
        ...
        # Get embeddings and visualize with PCA
        embeddings_2d, pca = embedder.visualize_embeddings_with_pca(descriptions, labels=short_labels)
        print(f"Saved PCA visualization for {strategy} pooling strategy")
