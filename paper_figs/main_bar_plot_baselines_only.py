...
def create_bar_plot(file_paths, scenario_names=None, system_names=None, output_file = ""):
    ...
    for 4 baseline systems (excluding your system).
    
    Args:
        file_paths: Dictionary of dictionaries with scenario and system as keys and file paths as values
                   Example: {'scenario1': {'deepsense': 'path/to/file1.csv', 'limu_bert': 'path/to/file2.csv', ...}, ...}
        scenario_names: List of names for the 3 scenarios (default: ['Scenario 1', 'Scenario 2', 'Scenario 3'])
        system_names: List of names for the 4 baseline systems (default: ['DeepSense', 'LIMU-BERT', 'UniHAR', 'ContraSense'])
    ...
    if system_names is None:
        # Only baseline systems, no "Your System"
        system_names = ['DeepSense', 'LIMU-BERT', 'UniHAR', 'ContraSense']
    
    # System keys in the dictionary - excluding your_system
    system_keys = ['deepsense', 'limu_bert', 'unihar', 'contrasense']
    
    # Color scheme - updated for 4 baseline systems only
    ...
    fig, axes = plt.subplots(1, 3, figsize=(11, 2.5))  # Reduced width since we have fewer bars
    ...
    for i, (scenario_key, ax) in enumerate(zip(scenario_keys, axes.flatten())):
        ...
        # Process each system's data for this scenario (excluding your_system)
        ...
        width = 0.25  # Slightly wider bars since we have fewer systems
...
if __name__ == "__main__":
    # Example usage with explicit file paths - excluding your_system
    file_paths = {
        'scenario1': {
    ...
    # Updated scenario and system names - excluding GestureLens
    scenario_names = ['Watch Hand Gesture (SU)', 'Earbud Hand Gesture (SU)', 'Blind User Hand Gesture (BU)']
    system_names = ['Deep-\nSense', 'LIMU-\nBERT', 'UniHAR', 'Contrast-\nSense']
    
    # Create and show the plot
    fig = create_bar_plot(file_paths, scenario_names, system_names, output_file='paper_figs/baselines_only_bar_plot.pdf')
