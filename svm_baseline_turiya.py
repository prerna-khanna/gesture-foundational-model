...
def load_dataset(dataset_name='Turiya', merge=20, label_rate=0.08, stratified_split=True):
    """
    Load dataset and apply same preprocessing as classifier_with_contrastive.
    
    Args:
        dataset_name: Name of user dataset (e.g., 'Turiya', 'Prerna')
...
def plot_confusion_matrix(cm, gesture_ids, save_path='results/svm_confusion_matrix_turiya.png'):
    """
    ...
    plt.title('SVM Classifier - Turiya Dataset Confusion Matrix')
    plt.tight_layout()
...
def main():
    print("="*70)
    print("SVM Baseline Classifier - Turiya Dataset")
    print("="*70)
    ...
    results_file = os.path.join(results_dir, f'svm_baseline_turiya_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt')
    with open(results_file, 'w') as f:
        f.write("="*70 + "\n")
        f.write("SVM Baseline Classifier - Turiya Dataset\n")
        f.write("="*70 + "\n\n")
